<?php
    if ($_SERVER['REQUEST_METHOD'] != "GET") {
        ...
        if($_SESSION['inicio_sesion']['cargo'] != "subdirector" && $_SESSION['inicio_sesion']['cargo'] != "jefe" && $_SESSION['inicio_sesion']['cargo'] != "coordinador"){
            ...
            $sentencia_carros = "SELECT  DISTINCT `placa` FROM `vehiculos` INNER JOIN  `movimientos` m ON  `placa` =  m.vehiculo WHERE  m.tipo_movimiento = 'ENTRADA' AND  `tipo` = 'carro' AND m.fecha_registro= (SELECT MAX(`fecha_registro`) FROM `movimientos` WHERE `vehiculo` = `placa`);";

...
if ($_SERVER['REQUEST_METHOD'] != "GET") {
    ...
    if ($_SESSION['inicio_sesion']['cargo'] != "jefe") {
        ...
        $sentencia_salidas = "SELECT documento, fecha_registro FROM `movimientos` WHERE `tipo_movimiento` = 'ENTRADA' ORDER BY `fecha_registro` DESC  LIMIT 4;";
        ...
        if (!$conexion) {
            ...
            if ($consulta->num_rows == 0) {
                $vector_respuesta = [
                    'titulo' => "ERROR",
                    'msj' => "No se encontraron registros de entradas",
                    'cod' => 404
                ];
            } else {
                // Almacenamos los resultados
                $entradas = $consulta->fetch_all();
                ...
                foreach ($entradas as $entrada) {
                    ...
                    foreach($tablas as $tabla){
                        ...
                        $sentencia_buscar = "SELECT v.nombres, v.apellidos, v.tipo_documento, v.documento, m.tipo_movimiento, m.fecha_registro, m.vehiculo FROM `$tabla` v INNER JOIN `movimientos` m ON v.documento = m.documento WHERE v.documento = '$entrada[0]' AND m.fecha_registro = '$entrada[1]';";
                ...
                $vector_respuesta = [
                    ...
                    'msj' => "Se encontraron registros de entradas",
                    'cod' => 200,
                    'entradas' => $todos_los_resultados
                ];

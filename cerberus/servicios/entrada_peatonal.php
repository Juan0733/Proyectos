            ...
            if (!$conexion) {
                ...
                foreach ($tablas as $tabla) {
                    ...
                    if ($buscar_usr && $buscar_usr->num_rows > 0) {
                        ...
                        if($ubicacion['ubicacion'] == "AFUERA"){
                            $ubicacion_correcta = true;
                        }
                        $documento_encontrado = true; 
                        $sentencia_update = "UPDATE `$tabla` SET `ubicacion` = 'ADENTRO' WHERE `documento` = '$numero_documento';";
                        break;
                    }
                }
                ...
                if(!$documento_encontrado){
                    ...
                    if(!$ubicacion_correcta){
                        $vector_respuesta = [
                            'titulo' => "ERROR",
                            'msj' => "¡Parece que esta persona no se le registro una salida!",
                            'cod' => 402,
                        ];
                    }else{
                        $sentencia_insertar_movimiento = "INSERT INTO `movimientos`(`documento`, `fecha_registro`, `usr_sistem`, `puerta_registro`, `tipo_movimiento`) VALUES ('$numero_documento','$momento_registro','$usr_sistema','peatonal','ENTRADA');";
                        ...
                        if($filas_afectadas != 1){
                            ...
                            if($filas_afectadas != 1){
                                ...
                                $vector_respuesta = [
                                    'titulo' => "REGISTRO",
                                    'msj' => "¡Entrada registrada con exitó!",
                                    'cod' => 200,
                                ];
                            }
